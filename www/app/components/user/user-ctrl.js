(function () {
    ...
    function userCtrl($location, $scope, $rootScope, service, activityService, $ionicModal, $state, $ionicHistory, authService, sharedService, $ionicPopover, eventService, modalService, $ionicPopup) {
        ...
        $scope.confirmAnswer = function (question, answer) {
            if(answer) {
                if (question.schema.format == "integer") {
                    ...
                    console.log("not integer or float");
                }
        ...
        function activate() {
            ...
            activityService.getPushupPercentage(id).then(function (pushupQuantile) {
                pushupQuantile = pushupQuantile * 100;
             ...
             activityService.getCooperPercentage(id).then(function (cooperQuantile) {
                cooperQuantile = cooperQuantile * 100;
        ...
        $scope.changeLimit = function (direction) {
            if(direction == 'more') {
                $scope.ui.activityLimitSize = $scope.ui.activityLimitSize + 5;
            } else {
                $scope.ui.activityLimitSize = 5;
            }
        };
        ...
        $scope.nextNotification = function () {
            $scope.ui.notificationIndex = $scope.ui.notificationIndex + 1;
            if(angular.isUndefined($scope.notifications[$scope.ui.notificationIndex]) && $scope.notifications[$scope.ui.notificationIndex] == null) {
