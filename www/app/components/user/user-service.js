(function () {
  ...
  userService.$inject = ['$rootScope', '$location', '$http', '$q',
    'activityService', '$ionicPopup', '$ionicLoading', 'authService',
    '$ionicHistory', '$state', 'sharedService'];

  function userService($rootScope, $location, $http, $q, activityService,
    $ionicPopup, $ionicLoading, authService, $ionicHistory,
    $state, sharedService) {
    var origin = 'http://resultp.jumar.io'; // TODO-DEBUG, never used?
          ...
          // get user activities
            activityService.getUserActivities(id).then(function (activities) {
              ...
              userData.activities.forEach(function (activity) {
                activity.formattedDate = sharedService.parseDateToFinnishFormat(
