(function () {
    ...
    eventCtrl.$inject = ['$state','$location', '$scope', '$stateParams', '$rootScope', '$ionicHistory','activityService', 'eventService', '$ionicModal', 'modalService', 'sharedService', '$ionicPopup'];

    function eventCtrl($state, $location, $scope, $stateParams, $rootScope, $ionicHistory, activityService, service, $ionicModal, modalService, sharedService, $ionicPopup) {
        ...
        $scope.addOrRemoveEvent = function(event) {
            /*service.addEventToStorage(event);
            $rootScope.events = service.getEvents();*/
            initEvent();
            $state.go('events');
        ...
        $scope.userAttendedEvent = function (event) {
            var result = event.participants.indexOf(localStorage.getItem('id'));
              if(result > -1) {
                return true;
              } else {
                return false;
